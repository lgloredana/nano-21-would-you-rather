...
* App Functionality . 
Users should have a way of impersonating/logging in as an existing user. (This could be as simple as having a login box that appears at the root of the application that lets the user select a name from the list of existing users. Alternatively, he/she could create his/her own account throught creation process that allowes a user to sign up for an account.) The application should work correctly regardless of which user is selected. Once the user logs in, the home page showes it.
